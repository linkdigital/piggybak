  ...
  class Cart
    ...
    def initialize(cookie='')
      ...
      cookie.split(';').each do |item|
        ...
        if item_sellable.present?
          self.sellables << { :sellable => item_sellable, :quantity => (item.split(':')[1]).to_i, :product_id => (item.split(':')[2]).to_i }
        end
      end
    ...
    def self.to_hash(cookie)
      ...
      cookie.split(';').inject({}) do |hash, item|
        hash["#{(item.split(':')[0]).to_s}p#{(item.split(':')[2]).to_s}"] = (item.split(':')[1]).to_i
        hash
      end
    end
    ...
    def self.to_string(cart)
      cookie = ''
      cart.each do |k, v|
        cookie += "#{(k.split('p')[0]).to_s}:#{v.to_s}:#{(k.split('p')[1]).to_s};" if v.to_i > 0
      end
      cookie
    end
    ...
    def self.add(cookie, params)
      cart = to_hash(cookie)
      params[:sellable_id].split(',').each do |item|
        cart["#{item}p#{params[:product_id]}"] ||= 0
        cart["#{item}p#{params[:product_id]}"] += params[:quantity].to_i
      end
      to_string(cart)
    end
